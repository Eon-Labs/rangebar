...
use rangebar_rust::{AggTrade, FixedPoint, RangeBarProcessor};
...
fn bench_memory_efficiency(c: &mut Criterion) {
    ...
    for batch_size in [1000, 10000, 100000].iter() {
        group.bench_with_input(BenchmarkId::new("batch_processing", batch_size), batch_size, |b, &size| {
...
fn bench_extreme_cases(c: &mut Criterion) {
    ...
    group.bench_function("high_volatility", |b| {
        b.iter(|| {
            let mut processor = RangeBarProcessor::new(8000);
            let bars = processor.process_trades(black_box(&high_volatility_trades)).unwrap();
            black_box(bars);
        });
    });
    ...
    group.bench_function("low_volatility", |b| {
        b.iter(|| {
            let mut processor = RangeBarProcessor::new(8000);
            let bars = processor.process_trades(black_box(&low_volatility_trades)).unwrap();
            black_box(bars);
        });
    });
