...
fn main() {
    ...
    let trades = vec![
        ...
        AggTrade {
            ...
            first_trade_id: 2, last_trade_id: 2,
        ...
        AggTrade {
            ...
            first_trade_id: 3, last_trade_id: 3,
        ...
        AggTrade {
            ...
            first_trade_id: 4, last_trade_id: 4,
        ...
        AggTrade {
            ...
            first_trade_id: 5, last_trade_id: 5,
    ...
    for trade in &trades {
        ...
        println!("  Trade: ${:.2} @ {:.1} volume", trade.price.to_f64(), trade.volume.to_f64());
    }
    ...
    println!("  Price Range: ${:.2} - ${:.2}", snapshot.price_stats.range.0, snapshot.price_stats.range.1);
    ...
    println!("  Volume Range: {:.2} - {:.2}", snapshot.volume_stats.range.0, snapshot.volume_stats.range.1);
    ...
    let range_bars = vec![
        RangeBar {
            open_time: 1609459200000, close_time: 1609459201000,
            ...
            turnover: 0, trade_count: 42, first_id: 1, last_id: 42,
            buy_volume: FixedPoint::from_str("6.0").unwrap(), buy_turnover: 0,
            sell_volume: FixedPoint::from_str("4.0").unwrap(), sell_turnover: 0,
            buy_trade_count: 25, sell_trade_count: 17,
            vwap: FixedPoint::from_str("50075.0").unwrap(),
        },
        RangeBar {
            open_time: 1609459201000, close_time: 1609459202000,
            ...
            turnover: 0, trade_count: 63, first_id: 43, last_id: 105,
            buy_volume: FixedPoint::from_str("9.0").unwrap(), buy_turnover: 0,
            sell_volume: FixedPoint::from_str("6.0").unwrap(), sell_turnover: 0,
            buy_trade_count: 38, sell_trade_count: 25,
    ...
    for bar in &range_bars {
        ...
        println!("  Bar: O=${:.2} H=${:.2} L=${:.2} C=${:.2} V={:.1}",
                bar.open.to_f64(), bar.high.to_f64(), bar.low.to_f64(),
                bar.close.to_f64(), bar.volume.to_f64());
    }
    ...
    println!("    Open Mean: ${:.2}, Std Dev: ${:.2}",
            final_snapshot.ohlc_stats.open.rolling.mean,
            final_snapshot.ohlc_stats.open.rolling.std_dev);
    println!("    High Mean: ${:.2}, Std Dev: ${:.2}",
            final_snapshot.ohlc_stats.high.rolling.mean,
            final_snapshot.ohlc_stats.high.rolling.std_dev);
    println!("    Low Mean: ${:.2}, Std Dev: ${:.2}",
            final_snapshot.ohlc_stats.low.rolling.mean,
            final_snapshot.ohlc_stats.low.rolling.std_dev);
    println!("    Close Mean: ${:.2}, Std Dev: ${:.2}",
    ...
    if price_mean >= 49000.0 && price_mean <= 51000.0 {
    ...
    if let (Some(p50), Some(p95)) = (
        ...
        if p50 < p95 {
            println!("  ✅ Percentiles are ordered (P50: {:.2} < P95: {:.2})", p50, p95);
        } else {
            println!("  ❌ Percentiles are not ordered (P50: {:.2} >= P95: {:.2})", p50, p95);
        }
    }

    // Check if counts match
    if final_snapshot.trade_count == trades.len() as u64 {
        println!("  ✅ Trade count matches ({} processed)", final_snapshot.trade_count);
    } else {
        println!("  ❌ Trade count mismatch (expected {}, got {})", trades.len(), final_snapshot.trade_count);
    }

    if final_snapshot.bar_count == range_bars.len() as u64 {
        println!("  ✅ Bar count matches ({} processed)", final_snapshot.bar_count);
    } else {
        println!("  ❌ Bar count mismatch (expected {}, got {})", range_bars.len(), final_snapshot.bar_count);
    }
